...
CloudAPI.prototype.createImageFromMachine = function(account, options, callback) {
    if (typeof (options) === 'function') {
        callback = options;
        options = account;
        account = this.account;
    ...
    if (options.machine && typeof (options.machine) !== 'string')
        throw new TypeError('options.machine must ba a string');

    if(options.machine && typeof (options.version) !== 'string')
        throw new TypeError('options.version must ba a string');
